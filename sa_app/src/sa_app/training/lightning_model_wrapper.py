...
class LightningModelWrapper(pl.LightningModule):
    def __init__(
        ...
        self.num_classes = (
            unique_config.get("training_params")
            .get("custom_classification_head")
            .get("num_labels")
        )
    ...
    def forward(self, x):
        ...
        [
            sentence_batch[inp_key].to(self.model.device)
            for inp_key, value in sentence_batch.items()
        ]
        ...
        output.loss = self.loss_fn(
            output.logits[:, 0, :], labels
        )  # Taking logit corresp. to CLS token
    ...
    def on_validation_batch_end(
        self, step_output: Dict, batch: Any, batch_idx: int, dataloader_idx: int = 0
    ) -> None:
        self.log_batch_end(
            step_output=step_output,
            batch=batch,
    ...
    def configure_optimizers(self):
        optimizer = Optimizer.from_config(
            params=self.parameters(), **self.optimizer_params
        )
